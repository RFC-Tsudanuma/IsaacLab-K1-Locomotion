...
"""Play a hierarchical goalkeeper agent.

Loads two checkpoints:

- ``--checkpoint``        : the trained high-level (goalkeeper) policy that outputs
...
parser = argparse.ArgumentParser(description="Play a hierarchical (goalkeeper) RL agent.")
...
# module (not train_goalkeeper.py, which would re-run its top-level argparse).
from goalkeeper_helpers import HierarchicalVecEnvWrapper, _build_frozen_policy


@hydra_task_config(args_cli.task, args_cli.agent)
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    """Play a trained hierarchical goalkeeper agent."""
    ...
    print(f"[INFO] Loading high-level (goalkeeper) checkpoint from: {resume_path}")
    if agent_cfg.class_name == "OnPolicyRunner":
        runner = OnPolicyRunner(hier_env, agent_cfg.to_dict(), log_dir=None, device=agent_cfg.device)
    else:
        raise ValueError(f"play_goalkeeper.py only supports OnPolicyRunner; got '{agent_cfg.class_name}'.")

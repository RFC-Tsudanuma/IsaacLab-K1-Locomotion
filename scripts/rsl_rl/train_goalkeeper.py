...
- The env's ``command_manager`` is left alone. It is expected to generate the
  high-level command the goalkeeper policy is asked to follow (e.g. goalkeeper target
...
To make this work cleanly with a goalkeeper env, the env cfg should expose a
...
Example (with the goalkeeper env's ``low_level`` group)::

    train_goalkeeper.py --task Isaac-Goalkeeper-K1-v0 \
...
parser = argparse.ArgumentParser(description="Hierarchical (goalkeeper) RL training with RSL-RL.")
...
from goalkeeper_helpers import HierarchicalVecEnvWrapper, _build_frozen_policy  # noqa: F401
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    if agent_cfg.class_name == "OnPolicyRunner":
        ...
        raise ValueError(
            f"train_goalkeeper.py only supports OnPolicyRunner; got '{agent_cfg.class_name}'."
        )
    runner.add_git_repo_to_log(__file__)

    dump_yaml(os.path.join(log_dir, "params", "env.yaml"), env_cfg)
    dump_yaml(os.path.join(log_dir, "params", "agent.yaml"), agent_cfg)
    with open(os.path.join(log_dir, "params", "goalkeeper_meta.txt"), "w") as f:
    ...
    # --- 上位 (goalkeeper) ポリシーのレジューム ---
    # --resume 指定時、--checkpoint (絶対/相対パス) か load_run/load_checkpoint から
    # 上位ポリシーの重み・オプティマイザ・学習イテレーション数を復元して継続学習する。
    # frozen (歩行) は別途 --frozen_checkpoint で読むので、ここでは触らない。
    # NOTE: runner.load は current_learning_iteration を復元し、
    #       runner.learn(N) は「そこから追加で N イテレーション」回す
    #       (--max_iterations は追加分。総数ではない)。
    if agent_cfg.resume:
        ...
        print(f"[INFO] Resuming high-level policy from: {resume_path}")
        runner.load(resume_path)
        print(f"[INFO] Resumed at iteration {runner.current_learning_iteration}; "
              f"will run {agent_cfg.max_iterations} more.")

...
#            加えて 2026-08-24 から **fewa/47b8863 (実機実証済み) の設定を丸ごと
#            持ち込む段** になった:
#              * 歩容 3 点 = 着地 shaping 3 項を無効化 / feet_phase 2.0 → 0.8 /
#                地形を NOISY_FLAT (random_rough 0.7 / plane 0.3、±1-4 cm)
#              * 観測パイプラインを fewa 方式へ = ボール位置・速度を同じ "vision"
#                group の連続遅延 0.02-0.08 s + 一様ノイズ ±0.07 m / ±0.5 m/s に、
#                IMU / エンコーダにも 0-0.02 s の遅延。継承元のガウス系パイプライン
#                (30 Hz サンプル&ホールド、位置スロットだけ) は捨てる。
#            **したがって「指標が動いたら原因は履歴」ではもう無い。** 束のどこが
#            効いているかは分からないまま、実機で動く構成に丸ごと合わせる段。
#            経緯は env cfg の K1WalkInsideKickDualEnvCfg の docstring
#            「観測パイプラインを fewa 方式へ (2026-08-24)」節。
#
...
#                                                     地形が厳しすぎる)
#   ただし kick_rate と kick_dir_error_deg (基準 4.2°) は、2026-08-24 の観測
#   張り替え (位置ノイズ ±0.02 m → ±0.07 m、速度 ±0.4 → ±0.5、IMU/エンコーダにも
#   遅延) で stage 2 の序盤が多少悪化するのが想定内。数百 iteration で戻らなければ
#   ボール位置ノイズが今の当て方には広すぎる (fewa も ±0.1 で崩れて ±0.07 に緩めた)。
#   観測の次元と並びは変わっていないので、引き継ぎ自体はこれまでどおり成立する。
#
# 使ってはいけないフラグ
# ----------------------

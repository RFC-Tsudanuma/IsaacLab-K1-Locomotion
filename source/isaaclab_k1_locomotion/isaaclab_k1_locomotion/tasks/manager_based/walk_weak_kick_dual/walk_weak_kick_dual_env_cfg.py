...
"""walk_weak_kick の dual 版 (dual encoder + 両足キック用の観測 2 変更)。
...
Stage 1 (walk phase) は :mod:`..walk_kick_dual` と共用::
    ...
    walk phase (walk_kick_dual) → weak → 360Weak (クリーン) → 360Weak DR (最終)
...
weak のレシピ (latch 閾値の指令追従 / kick_velocity_strong の折れ線 / σ アニール +
overshoot 罰 / ボール物性 DR) は継承元の ``__post_init__`` で全て済んでおり、
このファイルは **観測の見え方 + 報酬 5 項 (着地 shaping 3 + feet_phase + σ_direction)
しか触らない**。コマンド・ボール配置・終了条件は 1 バイトも変えていない。
...
学習の通し実行は ``scripts/rsl_rl/train_walk_kick_weak_dual.sh``。
"""
...
from ..walk_weak_kick.walk_weak_kick_env_cfg import (
    K1WalkKick360WeakEnvCfg,
    K1WalkKick360WeakEnvCfg_PLAY,
    K1WalkKickWeakEnvCfg,
    K1WalkKickWeakEnvCfg_PLAY,
)

# --------------------------------------------------------------------------- #
# DR 段 (stage 4) で定数化するフェードインの終点 [iteration]
#
# weak のカリキュラムのうち **フェードイン** は基底 walk_kick のキック報酬ランプ
# (0 → 500) と 360 の ball_avoidance_weight (0 → 500)。weak が足す 3 つは意図的に
# 後段に置かれているので触らない:
#
#   * kick_velocity_strong_weight : piecewise_reward_weight (立ち上げ→維持→フェードアウト)
#   * kick_velocity_scaled_sigma  : linear_reward_param (σ 1.0 → 0.35)
#   * kick_velocity_overshoot_weight : linear_reward_weight だが窓が 1500 → 3000
#
# _freeze_fade_in_curricula は func が linear_reward_weight の項だけを見て、さらに
# end_step > before_iter を除くので、上の 3 つは自動的に対象外になる。
#
# NOTE: つまり DR 段でも「strong を立ち上げてから落とす」移行はもう一度 0 から
#       回る。獲得済みの「指令どおりの強さ」に対しては余計だが、レシピを weak と
#       揃えることを優先した (既存の 1 フレーム版 stage 4 と同じ扱い)。
...
# --------------------------------------------------------------------------- #
# Stage 2 (weak, dual): 限定レンジ
# --------------------------------------------------------------------------- #
@configclass
class K1WalkKickDualWeakEnvCfg(K1WalkKickWeakEnvCfg):
    """Stage 2 (weak) の dual 版。

    継承元 (:class:`~..walk_weak_kick.walk_weak_kick_env_cfg.K1WalkKickWeakEnvCfg`)
    との差は both_feet の観測 2 変更 + 履歴入力 + 報酬の 2 調整だけ。weak のレシピ
    (latch 閾値の指令追従 / strong の折れ線 / σ アニール + overshoot / ボール物性 DR)
    は基底の ``__post_init__`` で済んでおり、そちらは以下に触らないので衝突しない:
...
@configclass
class K1WalkKickDualWeakEnvCfg_PLAY(K1WalkKickWeakEnvCfg_PLAY):
    observations: K1WalkKickBothFeetObservationsCfg = K1WalkKickBothFeetObservationsCfg()
...
# --------------------------------------------------------------------------- #
# Stage 3 (weak, dual): 全方位 (クリーン)。DR はまだ入れない
# --------------------------------------------------------------------------- #
@configclass
class K1WalkKickDual360WeakEnvCfg(K1WalkKick360WeakEnvCfg):
    """Stage 3 (weak, 全方位、クリーン) の dual 版。**最終段ではない** (次が DR 段)。
...
@configclass
class K1WalkKickDual360WeakEnvCfg_PLAY(K1WalkKick360WeakEnvCfg_PLAY):
    observations: K1WalkKickBothFeetObservationsCfg = K1WalkKickBothFeetObservationsCfg()
...
# --------------------------------------------------------------------------- #
# Stage 4 (weak, dual, 最終): 全方位 + 観測 DR
# --------------------------------------------------------------------------- #
@configclass
class K1WalkKickDual360WeakDREnvCfg(K1WalkKick360WeakEnvCfg):
    """Stage 4 (weak, 全方位 + 観測 DR) の dual 版。**この系列の最終 stage**。

    継承元は **クリーン 360** (:class:`~..walk_weak_kick.walk_weak_kick_env_cfg.K1WalkKick360WeakEnvCfg`)。
    Stage 3 (:class:`K1WalkKickDual360WeakEnvCfg`) と同じ土台に、
    ...
    **ボール観測 DR は一様ノイズ + 連続遅延** (fewa 47b8863 準拠)。ガウスの認識
    ...
    Stage 3 (dual) の checkpoint がそのまま載る::
        ...
        _labpython2 scripts/rsl_rl/train.py \
            --task Isaac-Velocity-Flat-K1-Walk-Kick-Dual-360-Weak-DR-v0 \
            --headless --num_envs 4096 --max_iterations 10000 \
            --load_pretrained logs/rsl_rl/k1_walk_kick_dual_360_weak/<run>/model_<N>.pt
    """
...
@configclass
class K1WalkKickDual360WeakDREnvCfg_PLAY(K1WalkKickDual360WeakDREnvCfg):
    def __post_init__(self) -> None:
        super().__post_init__()

...
def kick_foot_ceiling(
    ...
    * ``h ≥ h_cap + h_span`` → 0。
    ...
    .. note::
       **cfg 側が h_cap をどこに置くかで、この項の性格は「天井」にも
       「低いほど得」にも変わる。** 力学はどちらでも上の 1 本の式のままで、
       関数側に違いは無い。実際 :mod:`~...walk_inside_kick` は 2026-08-23 に
       h_cap = 0.09 (収束値のすぐ上 = 上限として) で入れたあと、2026-08-24 に
       **0.05 (収束値 0.087 より下 = 低く当てさせる圧として)** へ振り直している
       (:data:`~...walk_inside_kick.walk_inside_kick_env_cfg._FOOT_LOW_H_TARGET`)。
       下の「``h_cap = 0.09`` の根拠」は **初版の導出の記録**であって、
       この項の使い方をその値に縛るものではない。
    ...
    * ``touch_count > 0`` のゲートが **要る**。この項は凍結値の初期値 0.0 が
    ...
    Args:
        h_cap: これ以下なら満点になる足裏高さ [m]。既定 0.09 = 足箱の中心が
            ボール中心 (0.11) 以下に来る高さ = 「上限」としての置き方。
            **これより低く置けば「低く当てろ」の圧になる** (上の note)。
        h_span: 満点から 0 点になるまでの幅 [m]。既定 0.08 (h_cap = 0.09 なら
            h = 0.17 で 0)。
    """
    r_dir, state = _r_direction(
        env,

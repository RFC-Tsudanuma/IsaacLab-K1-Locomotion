...
def noisy_ball_pos_b(
    ...
    1. **遅延**: 真の相対位置を毎ステップリングバッファに積み、``delay`` ステップ前の値を
       ...
       env ごとに一様サンプルする。この関数の既定 (2, 6) は 50Hz 制御で 40-120ms
       だが、実際に学習で使う帯は cfg 側の
       :data:`~..walk_kick_env_cfg._BALL_OBS_DELAY_STEP_RANGE` (現在 (0, 6) =
       0-120ms) で、呼び出し元は全てこれを明示的に渡すのでこの既定値は使われない。
       ボールが静止している以上この遅延は「ボールが実際より遠く見える」形で効き、
       policy はそれを見越して早く振る補正を学習する。実機の認識が sim の帯より
       速いと手前で蹴る側に外れるので下端を 0 にした (経緯はあちらのコメント参照)。

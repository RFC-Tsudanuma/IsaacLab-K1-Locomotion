...
# 脚のアクチュエータ。
# NOTE: ここは **トルク-速度カーブ (T-N カーブ) 無し** の素の DelayedPDActuatorCfg。
#       全タスクの共通ロボット定義なので、実績のある既存タスク (walk_kick / walk_pass /
#       walk_loop_*) の物理を変えないためにこのままにしてある。
#       T-N カーブ付き (BoosterDelayedPDActuatorCfg) は walk_lob だけが
#       __post_init__ で差し替えて使う。詳細は locomotion/actuators.py の docstring。
...
K1_LOCOMOTION_CFG = ArticulationCfg(
    ...
    actuators={
        "legs": delayed_pd_leg,
        # 足首。legs と同じく T-N カーブ無し (walk_lob だけが差し替える)。

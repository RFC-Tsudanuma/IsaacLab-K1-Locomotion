...
class K1FlatTurnCfg(K1FlatStancePlaneCfg):
    ...
    **コマンド 3 枠に書き込む値の意味が変わる**。C++ 側は ``(vx, vy, ωz)`` ではなく
    ...
    def __post_init__(self):
        ...
        # μ の幅でカバーする方針。
        #
        #   学習でカバーされる τ (N=125 と仮定): r=0.051, μ=U(0.3, 3.5) -> 1.9 - 22.3 Nm
        #   実機で想定される τ (床 μ~0.6-1.0、実効半径 0.05-0.097): 3.8 - 12.1 Nm
        #   -> 実機想定は学習範囲の 9% - 50% に位置し、両側に余裕がある。
        #
        # 下限 0.3 は「滑りやすい床でも回りすぎない」ための余裕。上限 3.5 は
        # RMA 論文 (最大 4.5) の前例に倣った高摩擦側の余裕 (2026-09-16 に 2.8 から拡大)。
        # τ の上限 22.3 Nm は Hip_Yaw の出力上限 38.3 Nm の 58% なので、
        # 最高摩擦でも回れなくなることはない。
        #
        # NOTE: r を変えたら μ の範囲も必ず見直すこと (τ が連動して動くため)。
        self.events.physics_material.params["static_friction_range"] = (0.3, 3.5)
        self.events.physics_material.params["dynamic_friction_range"] = (0.3, 3.5)

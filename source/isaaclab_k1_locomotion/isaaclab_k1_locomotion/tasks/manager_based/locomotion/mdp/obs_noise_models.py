...
★ 2026-08-25: ``feat/inoue_walk_double_encoder`` から移植。あちらは歩行タスクで
  実機まで持っていった際に追加したもので、**実機由来の知見**が入っている。
  横移動タスクでは **遅延ステージだけ** を使う (下記 ``_USE_SENSOR_ARTIFACT_NOISE``
  相当の判断はあちらでも False。バイアス/EMA/ホールドは推測ベースで、
  実機の後退転倒に効かなかったと記録されている)。

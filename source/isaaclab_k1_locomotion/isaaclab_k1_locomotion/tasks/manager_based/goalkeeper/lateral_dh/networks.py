...
"""履歴つき観測から「直接入力 + 直近数ステップ + CNN 履歴特徴」を MLP に入力する ActorCritic。

★ 2026-08-23: ``feat/inoue_walk_double_encoder`` の
  ``locomotion/agents/history_actor_critic.py`` を横移動タスクへ移植したもの。
  クラス名だけ ``LateralHistoryActorCritic`` に変えて、あちらのブランチと
  ``eval(class_name)`` 名前空間が衝突しないようにしてある。中身の構造は同一。

環境側 (K1GKLateralDHEnvCfg) は以下の観測グループを出力する:

* ``direct``: 履歴なしの直接入力 (最新の速度指令 3 + GK タスクスロット 10)
* ``policy``:  actor 用の歩行観測 49 次元の HISTORY_LENGTH ステップ分の履歴 (ノイズあり)
* ``critic``:  critic 用 54 次元 (特権情報込み・ノイズなし) の同様の履歴
...
class LateralHistoryActorCritic(ActorCritic):
...
@configclass
class RslRlLateralHistoryActorCriticCfg(RslRlPpoActorCriticCfg):
    """LateralHistoryActorCritic 用の設定。"""

    class_name: str = "LateralHistoryActorCritic"
...
_rsl_rl_on_policy_runner.LateralHistoryActorCritic = LateralHistoryActorCritic

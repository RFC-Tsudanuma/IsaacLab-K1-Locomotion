...
class K1GKHierStage2EnvCfg(K1GKHierEnvCfg):
    ...
    def __post_init__(self):
        ...
        # --- 守備面: ゴールラインから 0.7m 前 (ユーザー指示 2026-08-15、0.8 → 0.7) ---
        # ★ この行は初期配置の導出より先に置くこと (下の _gx がこの値を読む)。
        #
        # 前に出るほど不利になる設計であることに注意。この課題ではボールが既に狙い先を
        # 決めて発射済みなので、実際のサッカーで言う「前に出て角度を狭める」利点が
        # 発生しない。一方で ①守備面を横切る点が外側へ散る ②反応時間が減る という
        # コストだけが乗る。実測 (到達不能率、成功率の分母から除外される割合):
        #     guard_x   初段(hi=1.0/aim0.4)   最終段(hi=6.0/aim1.1)
        #       0.9          15.1%                 43.8%
        #       0.8          10.4%                 39.9%
        #       0.7           6.2%                 35.9%
        self.goalkeeper.guard_x = 0.7

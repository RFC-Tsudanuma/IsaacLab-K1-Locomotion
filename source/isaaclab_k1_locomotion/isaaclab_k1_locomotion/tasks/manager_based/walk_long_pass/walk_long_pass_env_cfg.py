...
観測を 100 フレーム (2 秒) の履歴にする
--------------------------------------
policy 観測 (内界センサ + 前ステップの行動 + 受信コマンドの 55 次元) を毎ステップ
100 フレーム分バッファし、actor には

* 直近 5 フレームをそのまま (5 × 55 = 275)
* 100 フレーム全部を 1D-CNN で符号化した潜在 (16 × 15 = 240)

を連結して入れる (合計 515 次元)。CNN は隠れ層 2 つで
...
ONNX の入力も (1, 100, 55) に変わる。実機側は 55 次元の観測をリングバッファに
...
#
# 100 フレーム = 2.0 秒 (制御周期 = decimation 4 × sim.dt 0.005 = 0.02 s / 50 Hz)。
# 歩行 1 周期は _PHASE_FREQ 1.6 Hz で 0.625 秒なので、履歴には歩行 3 周期強が入る。
#
# policy 観測 (55 次元: 内界センサ + 前ステップの行動 + 受信コマンド) を毎ステップ
# 100 フレーム分バッファし、actor には
#   * 直近 5 フレームをそのまま
#   * 100 フレーム全部を 1D-CNN で符号化した潜在 (240 次元)
# を入れる。ネットワーク側は
...
# **項ごとに** (100, d_i) を平坦化してから連結するので、並びが
# [gravity の 100 フレーム][ang_vel の 100 フレーム]... になり、フレーム単位の
# (N, 100, 55) には戻せない (CNN のチャンネル = 観測次元にできなくなる)。
# False なら各項が (N, 100, d_i) のまま最終軸で連結され、(N, 100, 55) が得られる。
#
# 履歴は環境リセットで巻き戻り、直後は現在フレームで 100 個すべてが埋まる
# (IsaacLab の CircularBuffer の仕様)。ゼロ詰めの履歴は入らない。
#
# NOTE: ここを変えると rollout storage が num_steps_per_env × num_envs × H × 55 の
#       float で効いてくる (H=100・4096 env・48 step で約 4.3 GB、H=50 の倍)。
#       VRAM が足りなくなったら num_envs か num_steps_per_env を下げること。
...
def enable_obs_history(cfg) -> None:
    """policy 観測グループを 100 フレーム (2 秒) の履歴にする。
...
class K1WalkLongPassEnvCfg(K1WalkLoopPass360EnvCfg):
    ...
    100 フレームの観測履歴を見る (モジュール docstring の「観測を 100 フレーム (2 秒) の
    履歴にする」節)。
    """
    ...
    def __post_init__(self) -> None:
        ...
        # -- 0. Actor の観測を 100 フレーム (2 秒) の履歴にする
        #
        # 項の中身・順序・ノイズは継承元のまま。1 フレーム 55 次元が
        # (num_envs, 100, 55) になるだけで、切り出し方 (直近 5 + CNN 潜在) は

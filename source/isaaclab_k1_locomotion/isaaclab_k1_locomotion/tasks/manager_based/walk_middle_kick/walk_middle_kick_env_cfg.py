...
2026-08-24 の実機実験による検証 (large field / 帯の両端を実測)
--------------------------------------------------------------
large フィールド (22 m × 14 m、ゴールラインからペナルティエリア線 5 m /
ゴールエリア線 2 m) のセンターラインから蹴り、着地点を目視で記録した::

    指令 3.2 -> ペナルティエリア線とゴールエリア線の間 = センターから 6-9 m  (中央値 7.5)
    指令 4.5 -> ゴールエリア線とゴールの間           = センターから 9-11 m (中央値 10.0)

**戦略要求 (5-10 m) は満たしている。** 帯 (3.2, 4.5) の設定そのものは正しかった。
ただしこの 2 点は上の a ≈ 1.0 の較正に対して次の 2 つを突きつける。

* **上端は当たり、下端は外れた。** a = 1.0 の予測は 3.2 -> 5.1 m / 4.5 -> 10.1 m。
  4.5 の予測 10.1 m は実測 9-11 m の中に入る (較正が上端で妥当なことの追認)。
  一方 3.2 の予測 5.1 m は実測レンジ 6-9 m の **下に外れている** — 弱く蹴れと
  指令したのに予測より 1-4 m 余計に飛んだ。
* **単一の a では 2 点を説明できない。** 各点から a を逆算すると
  3.2 の側が a ∈ [0.57, 0.85]、4.5 の側が a ∈ [0.92, 1.13] で、
  **2 つの区間が重ならない**。転がり摩擦による減速は速度にほぼ依らないはずなので、
  これは「a の推定誤差」ではなく **指令 v_target と実蹴り速度 v_ball の関係の方が
  歪んでいる** ことを意味する。

指令に対する実蹴り速度の圧縮 (最重要)
--------------------------------------
飛距離は v² に比例するので、指令が 3.2 -> 4.5 (1.41 倍) なら飛距離は
1.41² = **1.98 倍** になるはずである。ところが実測の飛距離比は
10.0 / 7.5 = **1.33 倍** しかない。a = 1.0 を固定して実蹴り速度を逆算すると::

    指令 3.2 -> v_ball ≈ sqrt(2 × 1.0 × 7.5)  = 3.87 m/s  (追従率 1.21 = 出しすぎ)
    指令 4.5 -> v_ball ≈ sqrt(2 × 1.0 × 10.0) = 4.47 m/s  (追従率 0.99 = ほぼ指令どおり)

つまり **指令幅 1.3 m/s に対して実速度の幅は 0.6 m/s しかなく、回帰の傾きは
約 0.46**。ポリシーは指令の強弱をほぼ無視して「だいたい同じ強さ」で蹴っている。

これは平均追従率 (``kick_vel_ratio``) では絶対に見つからない欠陥である。
低指令で出しすぎ (1.21) と高指令で足りない (0.99) が打ち消し合い、平均は
1.0 付近の健全な値に見えてしまう。**評価は必ず指令 v_target と実測 v_ball の
回帰の傾き (理想 1.0) で見ること**。

帯を広げるときの含意
--------------------
* 上端: 指令 4.5 で実測も 4.5 m/s 前後。**ここが実力の天井に近い**ので、
  帯の上端を 5 m/s より上へ置くと ``kick_velocity_scaled`` が上端付近で
  常に 0 点になり、その指令域の勾配が消える (「届かないなら常に全力」へ
  collapse する)。
* 下端: 圧縮の主因は低指令側なので、帯を下へ広げるほど傾きは悪化する。
  帯幅を広げるなら σ_velocity の固定値 (このタスクでは 0.5) をやめ、
  **指令相対の σ** (例 ``clamp(0.12 × v_target, 0.15, 0.5)``) にしないと、
  弱い指令の識別性が確保できない。

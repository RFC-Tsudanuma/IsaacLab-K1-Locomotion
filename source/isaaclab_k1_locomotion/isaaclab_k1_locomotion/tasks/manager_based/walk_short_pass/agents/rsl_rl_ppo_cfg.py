...
#
# 履歴長 H = 50 は環境側 (walk_short_pass_env_cfg._OBS_HISTORY_LENGTH) が決め、
...
def _use_history_cnn_policy(cfg) -> None:
    ...
    short_pass 系列の 4 段すべてがこれを呼ぶ。**1 段でも呼び忘れると、そこで
...
@configclass
class K1WalkShortPassWalkPhasePPORunnerCfg(K1WalkKickWalkPhasePPORunnerCfg):
    """Stage 1 (歩行のみ) の履歴入力版。"""

    def __post_init__(self):
        super().__post_init__()

        self.experiment_name = "k1_walk_short_pass_walk_phase"
        _use_history_cnn_policy(self)


@configclass
class K1WalkShortPassLoopPassPPORunnerCfg(K1WalkLoopPassPPORunnerCfg):
    """Stage 2 (限定レンジのループパス) の履歴入力版。"""

    def __post_init__(self):
        super().__post_init__()

        self.experiment_name = "k1_walk_short_pass_loop_pass"
        _use_history_cnn_policy(self)


@configclass
class K1WalkShortPassLoop360PPORunnerCfg(K1WalkLoopPass360PPORunnerCfg):
    """Stage 3 (全方位ループパス) の履歴入力版。"""

    def __post_init__(self):
        super().__post_init__()

        self.experiment_name = "k1_walk_short_pass_loop_360"
        _use_history_cnn_policy(self)


@configclass
class K1WalkShortPassPPORunnerCfg(K1WalkLoopPass360PPORunnerCfg):
    """Stage 4 (ショートパス本体)。actor は 50 フレームの観測履歴を見る。
    ...
    .. note::
        ...
        (:class:`K1WalkShortPassLoop360PPORunnerCfg`, experiment_name
        ``k1_walk_short_pass_loop_360``)。共用タスクの checkpoint から
    ...
    def __post_init__(self):
        ...
        self.experiment_name = "k1_walk_short_pass"
        _use_history_cnn_policy(self)

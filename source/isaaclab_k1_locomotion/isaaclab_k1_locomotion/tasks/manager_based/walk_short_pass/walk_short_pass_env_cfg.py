...
継承元が満点を取れる (2.0, 3.0) から目標の (0.5, 2.0) へ動かす。各時点で
...
# NOTE: これは **カリキュラムの終点**。開始時は _SHORT_PASS_SPEED_RANGE_START。
...
#       (下の -- 7) は start_range → end_range を線形に動かすだけなので、ここでは
#       帯を **下向き** に動かすことになる。移植元 (long_pass) の「今の実力の
#       少し上を指令し続ける」という設計意図とは向きが逆で、kick_rate ゲートは
#       「弱く蹴れているか」を見る形になる。
# NOTE: latch のトリガー速度 _SHORT_PASS_V_THRESH = 1.5 は、この帯の上限 2.0 に
#       対して高すぎる。指令 0.5-1.5 のキックは実蹴りが 1.5 m/s に届かないので
#       latch せず、kick_finished も成立しない可能性が高い (= キック報酬が 1 度も
#       payout されない)。帯を下げるなら v_thresh も下げる必要がある (目安 0.4 前後)
#       が、今回は「指令の範囲だけを変える」という指示なので **未変更** にしてある。
# --------------------------------------------------------------------------- #
...
#   * 2500 iteration かけるのは、帯の下限が 2.0 → 0.5、上限が 3.0 → 2.0 と
#     下向きに 1.0-1.5 m/s 動くため。1 iteration あたり 0.0004-0.0006 m/s ずつで、
#     ポリシーが追従する余裕を取る。
...
# --------------------------------------------------------------------------- #
_SHORT_PASS_SPEED_RANGE_START = (2.0, 3.0)
...
# NOTE: 上の理屈は移植元の帯 (3.2-5.0) 用で、このタスクの帯 (0.5-2.0) には
#       合っていない。1.5 は帯の上限 2.0 すら 0.75 倍で、指令 0.5-1.5 のキックは
#       latch も kick_finished も成立しない可能性が高い。帯に合わせるなら 0.4 前後
#       だが、「指令の範囲だけを変える」指示のため据え置いてある。
# --------------------------------------------------------------------------- #
...
# 開始帯の上限 v_target=3.0 のゲート (1.80) を十分通過できるようにするため
# (0.8 だと 2.40 で、初期の蹴りの大半が半減以下になり立ち上がりが遅れる)。
...
class K1WalkShortPassEnvCfg(K1WalkLoopPass360EnvCfg):
    ...
    def __post_init__(self) -> None:
        ...
        # 終点 _SHORT_PASS_SPEED_RANGE へはカリキュラム (下の -- 6) が動かす。
        ...
        # -- 7. 目標ボール速度の帯を (2.0,3.0) → (0.5,2.0) へ動かす
        #

...
"""K1 ウォークロングパス環境 + 回り込み G（5-10 m の強い転がしパス専用ポリシー）。
...
学習手順 (Stage 1-4 を通しで)::
    ...
    # Stage 1-4 を順に (専用の通しスクリプトはまだ無いので手で回す)
    # Stage 4 だけやり直す場合 (履歴入力版 Stage 3 の checkpoint から):
    _labpython2 scripts/rsl_rl/train.py \
        --task Isaac-Velocity-Flat-K1-Walk-Long-Pass-Orbit-v0 \
        --headless --num_envs 4096 \
        --load_pretrained logs/rsl_rl/k1_walk_long_pass_orbit_loop_360/<run>/model_<N>.pt
...
def delayed_prev_ball_pos_b(
    ...
    master 側では both_feet 系の 2 スロット構成に作り替えられて
...
def _apply_v_thresh(cfg: "K1WalkLongPassOrbitEnvCfg", v_thresh: float) -> None:
...
@configclass
class K1WalkLongPassOrbitEnvCfg(K1WalkLoopPass360EnvCfg):
    """ロングパス専用。
    ...
    def __post_init__(self) -> None:
        ...
        # -- 8. ボールまわりの 4 点 DR (足の反発 / ボール物性 / 初期回転 / 転がり減速)
        #
        # 値と理由は :mod:`.orbit_mods` のコメント参照。報酬にもコマンドにも観測にも
        # 触らないので、checkpoint はそのまま繋がる。
        apply_ball_param_dr(self)

        # -- 9. 回り込み型の G と、キック線を跨ぐときの遊びを全項に配る
        #
        # **報酬項を追加し終えた後 (= __post_init__ の最後) に呼ぶこと。**
        # G と overshoot 判定はその step で最初に kick_state を呼んだ項の値で確定するので、
        # 1 つでも取りこぼすと結果が報酬項の評価順に依存する (-- 5 の _apply_v_thresh と同じ)。
        apply_orbit_params(self)


@configclass
class K1WalkLongPassOrbitEnvCfg_PLAY(K1WalkLongPassOrbitEnvCfg):
    def __post_init__(self) -> None:
        super().__post_init__()

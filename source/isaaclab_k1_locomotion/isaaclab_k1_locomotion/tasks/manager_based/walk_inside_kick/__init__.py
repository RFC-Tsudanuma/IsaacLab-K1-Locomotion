...
#
# 2026-08-24: stage 2 は fewa/47b8863 (実機実証済み) の設定を丸ごと持ち込む段になった
# (歩容 3 点 + **観測パイプラインごと fewa 方式へ**: ボール位置・速度を同じ vision
# group の連続遅延 0.02-0.08 s + 一様ノイズ ±0.07 m / ±0.5 m/s、IMU / エンコーダにも
# 0-0.02 s の遅延)。継承元も stage 1 の本命ではなく Clean (ガウス系パイプライン無し)
# に張り替えてある。観測の次元と並びは変わらないので checkpoint はそのまま繋がる。
# --------------------------------------------------------------------------- #
gym.register(
